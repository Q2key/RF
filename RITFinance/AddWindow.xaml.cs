...
namespace RITFinance
{
    ...
    {
        private readonly UiСontroller _controller;
        ...
        private bool AddAssert()
        {
            ...
            {
                if (UiСontroller.StringCheker(TextBoxAmountC.Text) == "" || TextBoxValuteC.Text == "")
                {
                    MessageBox.Show("Заполните поля со звездочками!");
                    return false;
                }
                _controller.CreateCashAsset(TextBoxNameC.Text, UiСontroller.StringCheker(TextBoxAmountC.Text),
                    TextBoxValuteC.Text, TextBoxAdditionalInfoC.Text);
                return true;               
            }
